			...
			if (SCHE_ID == null || SCHE_ID == "") {
				response.getWriter().print(
						"{\"rows\":"
								...
								+ "}");
				String rString = DBUtil.select(DBUtil
						.getQuerySql(ht, "MAIL_ROUTER",
								createTime1, createTime2));//logger
				logger.warn("rString"+rString);
				...
				response.getWriter().print(
						"{\"rows\":"
								+ DBUtil.select(DBUtil.getQuerySql(ht,
										...
										createTime2)) + "}");
				String rsString = DBUtil.select(DBUtil.getQuerySql(ht,
						"SCHE_BATCH_FOR_ROUTER", createTime1,
						createTime2));
				logger.warn("rsString"+rsString);
			}
